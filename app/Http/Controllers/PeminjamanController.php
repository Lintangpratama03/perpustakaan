...
class PeminjamanController extends Controller
{
    ...
    public function index_panjang()
    {
        $pinjam = peminjaman::select('peminjaman.*', 'users.name as name_user')
            ->where('peminjaman.is_deleted', 0)
            ->where('peminjaman.status_perpanjang', 1)
            ->leftJoin('users', 'peminjaman.id_card', '=', 'users.id_card')
            ->get();
        // dd($pinjam);
        return view('peminjaman.panjang', compact('pinjam'));
    }

    public function edit_panjang($id)
    {
        // dd($id);
        $buku = keranjang::select('keranjang.*', 'buku.name', 'buku.image')
            ->where('id_peminjaman', $id)
    ...
    public function edit_scan_panjang($id)
    {
        $buku = keranjang::select('keranjang.*', 'buku.name', 'buku.image')
            ->where('id_peminjaman', $id)
            ->leftJoin('buku', 'keranjang.id_buku', '=', 'buku.id')
    ...
    public function scan_panjang($id, $id_card)
    {
        // Menggunakan first() untuk mendapatkan hasil pertama atau null jika tidak ditemukan
        $pinjam = peminjaman::where('id', $id)
            ->where('id_card', $id_card)
            ->where('status_perpanjang', 1)
            ->first();
        ...
        // Tambahkan 7 hari pada tenggat_kembali
        $tenggatBaru = Carbon::parse($pinjam->tenggat_kembali)->addDays(7);

        // Ubah status peminjaman menjadi 3 (perpanjangan diterima)
        $pinjam->tenggat_kembali = $tenggatBaru;
        $pinjam->status_perpanjang = 2; // Perpanjangan diterima
        $pinjam->save();

        return response()->json([
            'status' => 200,
            'message' => 'Berhasil mengubah status peminjaman dan memperpanjang tenggat pengembalian.'
        ]);
    }
}
